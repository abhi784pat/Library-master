...
<html>
	...
	<body>
		...
		<?php
			$query = $conn->prepare("SELECT book_isbn FROM book_issue_log WHERE member = ?;");
			...
			else
			{
				...
				for($i=0; $i<$rows; $i++)
				{
					...
					if($isbn != NULL)
					{
						$query = $conn->prepare("SELECT title, author, category FROM book WHERE isbn = ?;");
						...
						$query = $conn->prepare("SELECT due_date FROM book_issue_log WHERE member = ? AND book_isbn = ?;");
			...
			if(isset($_POST['b_return']))
			{
				...
				for($i=0; $i<$rows; $i++)
					if(isset($_POST['cb_book'.$i]))
					{
						...
						$query = $conn->prepare("DELETE FROM book_issue_log WHERE member = ? AND book_isbn = ?;");
						$query->bind_param("ss", $_SESSION['username'], $_POST['cb_book'.$i]);
						if(!$query->execute())
						...
						if($days > 0)
						{
							...
							if($price < $penalty)
								$penalty = $price;
							$query = $conn->prepare("UPDATE member SET balance = balance - ? WHERE username = ?;");
				...
				if($books > 0)
				{
					...
					$query = $conn->prepare("SELECT balance FROM member WHERE username = ?;");
					$query->bind_param("s", $_SESSION['username']);
					$query->execute();
